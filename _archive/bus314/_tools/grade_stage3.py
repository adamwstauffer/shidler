"""BUS-314 Stage 3 (post-build spec) grading scanner.

Walks the Stage 3 submissions directory, dedupes by student ID (keeping the
latest timestamp per ID), inspects each spec deliverable (.md / .docx / .pdf),
and produces a grading worksheet with rubric signals and tentative auto-scores.

Mirrors grade_stage2.py in shape, but the rubric is the four-criterion spec
rubric defined in `accounting-ratios/stage3-spec-assignment.md`:

    Clarity & Professionalism      /1
    Analytical Logic               /1
    Completeness & Model Review    /1
    Reproducibility                /1
                                   = /4 total
...
STAGE3_DIR = Path(
    ...
    r"\ignore-term\2026 Spring\stage3"
)
...
# Required sections (label keywords expected to appear as headings/labels)
REQUIRED_SECTIONS = [
    ("problem_statement", ["problem statement"]),
    ("inputs",            ["inputs", "known variables"]),
    ("named_ranges",      ["named range", "naming convention"]),
...
# Same six categories as Stage 2 (for analytical-logic coverage)
CATEGORY_KEYWORDS = {
...
# Named-range conventions — same prefixes as Stage 2
PREFIX_PATTERNS = {
...
MARKET_TOKEN_RE = re.compile(
    r"\b(share_price|shares_outstanding|cost_capital|tax_rate|"
    r"market_capitalization)\b",
    re.IGNORECASE,
)
...
def find_spec_file(folder: Path) -> Path | None:
    ...
    (matches Stage 2 heuristic — usually the canonical submission).
    """
...
def _section_words_after(text: str, anchors: list[str], limit: int = 4000) -> int:
    """Word count between an anchor and the next H1/H2 heading (or end).

    Many specs use ``### What Worked`` style H3 sub-headings inside the Model
    Review section, so we only stop at H1 or H2. We also start counting after
    the anchor's own heading line to avoid double-counting the title.
    """
    lower = text.lower()
    best_idx = -1
    for a in anchors:
    ...
    if best_idx == -1:
        return 0
    # Skip past the rest of the line containing the anchor (its own heading).
    nl = text.find("\n", best_idx)
...
def inspect(sub: Submission) -> Grade:
    ...
    g.market_token_hits = len(set(m.lower() for m in MARKET_TOKEN_RE.findall(text)))
    ...
    #    >=4 of 6 categories AND calculation_flow section present
    has_calc = "calculation_flow" in found_sections
    g.auto_logic = 1 if (len(cats_found) >= 4 and has_calc) else 0

    # 3. Completeness & Model Review /1
    #    >=6 of 8 sections AND model review > 100 words
    ...
    # 4. Reproducibility /1
    #    >=10 distinct named-range tokens AND >=3 of 5 prefix families
    g.auto_reproducibility = 1 if (
        g.distinct_named_tokens >= 10 and len(prefixes_hit) >= 3
    ) else 0

    # ----- Flags -----
    if g.word_count < 50:
        g.flags.append("STUB")
    missing = [k for k, _ in REQUIRED_SECTIONS if k not in found_sections]
    if missing:
        g.flags.append("MISSING_SECTION:" + ",".join(missing))
    if len(cats_found) < 4:
        g.flags.append("MISSING_CATEGORY")
    if g.distinct_named_tokens < 10:
...
TOTAL_POINTS = 4  # spec rubric is /4
...
def _write_curve_formulas(ws, final_col: int, rank_col: int,
    ...
    Curved /4 (80% floor): top -> bottom
        Q1: 4.00 -> 3.75  (drops 0.25)
        Q2: 3.75 -> 3.50  (drops 0.25)
        Q3: 3.50 -> 3.35  (drops 0.15)
        Q4: 3.35 -> 3.20  (drops 0.15)
        Bottom of curve = 3.20 = 80% of 4.

    Curved 90% /4 (generous variant):
        Q1: 4.00 -> 3.90
        Q2: 3.90 -> 3.80
        Q3: 3.80 -> 3.70
        Q4: 3.70 -> 3.60
    ...
    for r in range(first_row, last_row + 1):
        ...
        # 80% floor curve: 4.00 -> 3.75 -> 3.50 -> 3.35 -> 3.20
        floor80 = (
            f'IF({p}<=0.25,4-{p}*1,'
        ...
        # 90% floor curve (generous): 4.00 -> 3.90 -> 3.80 -> 3.70 -> 3.60
        floor90 = (
            f'IF({p}<=0.25,4-{p}*0.4,'
...
def build_worksheet(grades: list[Grade]) -> None:
    ...
    headers = [
        ...
        "Named Prefix Families", "# Distinct Named Tokens", "# Market Tokens",
        "Model Review Words",
    ...
    for g in grades:
        ...
        row = [
            ...
            None, None, None, None,  # Rank, Quartile, Curved, Curved 90% — formulas below
            ", ".join(g.flags),
            g.error,
        ]
        ws.append(row)

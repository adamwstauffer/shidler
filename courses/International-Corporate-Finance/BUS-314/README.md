# BUS 314: International Business Finance

**Shidler College of Business, University of Hawaiʻi at Mānoa**

> **Inactive.** No live undergraduate offering today. This project design is superseded by the shared Performance Ratios project (BUS 629's current design) at [`../projects/performance-ratios/`](../projects/performance-ratios/). Historical project materials (stage docs, templates, master spreadsheet, grading scripts) are preserved at [`_archive/bus314/`](../../../_archive/bus314/), not maintained. If an undergraduate offering resumes, it would use the shared `projects/performance-ratios/` design rather than reviving this one.

## Course Overview

This course develops students' understanding of corporate finance. It introduces financing and investment decision-making considerations and the securities available to financial managers. The course relies heavily on developing models using Microsoft Excel.

---

## Instructor

**Adam W. Stauffer** | [adamstau@hawaii.edu](mailto:adamstau@hawaii.edu)

Please begin email subject lines with **BUS 314**. For more information about the instructor, see [BIO.md](../../../BIO.md).

---

## Textbook

Brealey, Myers, and Marcus, *Fundamentals of Corporate Finance*, 11th ed., McGraw-Hill

* Access to **McGraw-Hill (MH) Connect** web-based course companion system.

You can register for MH Connect on **Lamaku** by selecting the **“MH Campus”** option from the left menu.

**Optional Financial Calculator:**
HP 12C Financial Calculator

---

## Slides

Slides will be available on Lamaku:
[https://lamaku.hawaii.edu/portal](https://lamaku.hawaii.edu/portal)

---


# Prerequisites

* ACC 200
* ACC 210

---

## Topics Covered

* Goals & Governance of the Corporation
* Financial Markets & Institutions
* Accounting & Finance
* Measuring Corporate Performance
* The Time Value of Money
* Valuing Bonds
* Valuing Stocks
* Net Present Value & Other Investment Criteria
* Using DCF Analysis to Make Investment Decisions
* Risk, Return & the Opportunity Cost of Capital & Capital Budgeting
* WACC & Company Valuation
* Introduction to Corporate Financing
* How Companies Raise Venture Capital & Issue Securities
* Long-term & Short-term Financial Planning
* Options

---

## Course Outline

The course progresses through foundational concepts of corporate finance, starting with accounting and financial analysis, moving through valuation methods (bonds and stocks), and concluding with capital budgeting, cost of capital, and advanced topics in financing and risk management. Throughout, emphasis is placed on real-world applications using spreadsheet modeling and case studies.

---

# Administration

## Registration

Students with registration issues should contact the **Office of Student Academic Services (OSAS)** in BusAd B101.

More information:
[http://www.shidler.hawaii.edu/current-students/contact](http://www.shidler.hawaii.edu/current-students/contact)

---

## Textbook / IDAP Program

This course participates in the Bookstore’s **Interactive Digital Access Program (IDAP)**.

* Course materials will be available digitally via Lamaku by the first day of class.
* A charge for digital materials will be added to your MyUH account.
* You may opt out via the Inclusive Access Student Portal (linked in your IDAP welcome email).
* If you opt out, access will be removed and charges refunded.
* Unpaid charges may result in a hold on your account.

More information:
[https://www.bookstore.hawaii.edu/manoa/site_IDAP.asp](https://www.bookstore.hawaii.edu/manoa/site_IDAP.asp)

---

## Communication

* Email is the preferred method in emergencies.
* Check the Lamaku course website one day before each class session.

---

# Grading

| Description                | Percent  |
| -------------------------- | -------- |
| Attendance & Participation | 10%      |
| Weekly Homework            | 20%      |
| Individual Project #1      | 10%      |
| Individual Project #2      | 10%      |
| Midterm Exam               | 25%      |
| Final Exam                 | 25%      |
| **Total**                  | **100%** |

---

## Attendance and Participation

Attendance will be taken and factored into your participation score.

---

## Homework

* Administered through McGraw-Hill Connect
* Due Fridays by midnight

---

## Individual Projects

1. **Retirement Objectives (Excel Exercise)**
   See: BUS314 Mini Project – Retirement Objectives
2. **Agentic AI in Corporate Finance (or your chosen career)**
   Assignment details pending

---

## Examinations

* Midterm and Final Exam
* Held online via MH Connect
* Open-book

Make-up exams:

* Rare and only under legitimate, unavoidable circumstances
* Prior approval required
* Documentation required
* May be more rigorous than original exam

If you believe there is a grading error, notify the instructor within one week of posting. If regrading is requested, the entire exam/homework will be regraded.

---

# Accessing McGraw-Hill Connect

Register via Lamaku by selecting **“MH Campus.”**
You should not need to enter payment details.

An optional discounted loose-leaf version of the textbook is available at the bookstore.

---

# Course Compliance with Campus Policies

Students must follow all rules and policies of the University of Hawaiʻi at Mānoa and the Shidler College of Business.

---

## Students with Disabilities

Students with documented disabilities should contact the instructor or the **KOKUA Program** (Student Services Center, Room 13, 956-7511).
Website: [http://www.hawaii.edu/kokua/](http://www.hawaii.edu/kokua/)

---

# Academic Honesty

Cheating and plagiarism will not be tolerated and will be handled under the UH Student Code of Conduct.

Full code:
[http://www.hawaii.edu/student/conduct](http://www.hawaii.edu/student/conduct)

---

## Definitions

### Cheating Includes:

* Unauthorized assistance during exams
* Obtaining exam information beforehand
* Submitting another’s work as your own
* Using prohibited materials
* Fabricating or falsifying data
* Altering grades or exam answers
* Falsifying University records
* Misrepresenting facts for exemptions

### Plagiarism Includes:

* Copying work without attribution
* Improper paraphrasing or quotation
* Submitting the same work in multiple courses without permission
* “Drylabbing” (using others’ experimental data or write-ups)

---

## Directory Contents

```
BUS-314/
└── README.md   this file — the offering is inactive, so there's nothing else live here
```

Full historical project materials (the superseded 4-stage design) are archived at [`_archive/bus314/`](../../../_archive/bus314/):

```
_archive/bus314/
├── _spreadsheets/                  Master Ratios Spreadsheet (legacy — superseded by docs/spreadsheets/Corporate Finance Master Spreadsheets.xlsx)
├── _templates/                     temp.md, template-decision-memo.md, template-spec.md
├── _tools/                         grade_stage2.py, grade_stage3.py, grade_stage4.py (legacy grading scripts)
└── accounting-ratios/               the 4-stage project itself
    ├── _scenarios/                  per-company scenario workbooks (AAPL, CAT, MSFT, WMT, XOM)
    ├── _templates/                  excel/ skeletons, example-spec-template.md
    ├── archive/                     analysis/, deliverables/, prompts/, specs/ from past iterations
    ├── BUS314_AI_Ratios_Project.pptx
    ├── README.md
    ├── company-selection.md, extra-credit.md
    └── stage1-memo-assignment.md … stage4-final-analysis-assignment.md
```

---


...
1. **Verify-don't-trust FIRST — the highest-value step.** Enumerate the claims the recommendation *rests on* and check each against this repo's ground truth before endorsing anything:
   ...
   - **Grading claims** ("this curve never lowers a raw score", "no double-deduction across stages") → check against the ai-lms `grading` skill (`C:\GitHubi-lms\.claude\skills\grading\`, moved 2026-09-16) and the relevant `feedback_*` memory (`feedback_grading_curves`, `feedback_no_double_deductions`, `feedback_regrade_policy`).
